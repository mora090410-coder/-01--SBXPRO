...
const OrganizerProof = () => (
  <div className="grid gap-4" aria-label="Organizer demo">
    <div>
      <p className="text-xs font-bold uppercase tracking-[0.2em] text-gold">For organizers</p>
      <h2 className="mt-1 text-2xl font-black tracking-tight text-broadcast-white">Set up the board</h2>
      <p className="mt-2 text-sm leading-relaxed text-broadcast-white/75">Add names → Check OPEN squares → Draw numbers → Preview → Publish</p>
    </div>
    <div className="rounded-2xl border border-broadcast-white/15 bg-broadcast-white/95 p-3 text-ink shadow-2xl">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-ink/60">Chiefs at Eagles · 42 assigned · 58 OPEN</p>
        </div>
        <span className="rounded-full bg-live-green/15 px-3 py-1 text-xs font-black text-live-green">Ready to preview</span>
      </div>
      <div className="grid grid-cols-10 gap-1" aria-label="Sample organizer board preview">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px] font-bold">
        <div className="rounded-xl bg-ink/5 p-2">Payment notes stay private</div>
        <div className="rounded-xl bg-ink/5 p-2">Numbers drawn</div>
        <div className="rounded-xl bg-ink/5 p-2">Share link ready</div>
      </div>
    </div>
  </div>
);
...
const ViewerProof = () => (
  <div aria-label="Game day demo" className="overflow-hidden rounded-2xl border border-broadcast-white/15 bg-ink">
    <div className="border-b border-broadcast-white/15 p-4">
      <p className="text-xs font-bold uppercase tracking-[0.2em] text-gold">For everyone watching</p>
      <h2 className="mt-1 text-2xl font-black tracking-tight text-broadcast-white">Follow on game day</h2>
      <p className="mt-2 text-sm text-broadcast-white/75">Find your squares, check the current result, and see what the next score would change.</p>
    ...
    <div className="max-h-[560px] overflow-auto">
      <Suspense fallback={<p className="p-4 text-broadcast-white">Opening the game day view…</p>}>
        <ViewerShell
          game={demoGame}
          board={demoBoard}
          live={demoLive}
...
export default function HomepageProofArtifact() {
  ...
  return (
    <section data-testid="homepage-proof-artifact" className="rounded-surface border border-broadcast-white/10 bg-broadcast-white/[0.04] p-3 shadow-2xl md:p-5" aria-label="GridOne demo board">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <p className="rounded-full border border-gold/40 bg-gold/10 px-3 py-2 text-xs font-black uppercase tracking-[0.16em] text-gold">Demo board — sample names and scores</p>
        <div className="flex gap-2" aria-label="Choose a demo view">
          <TouchButton active={mode === 'organizer'} onClick={() => setMode('organizer')}>Organizer view</TouchButton>
          <TouchButton active={mode === 'viewer'} onClick={() => setMode('viewer')}>Game day view</TouchButton>
        </div>
      </div>
