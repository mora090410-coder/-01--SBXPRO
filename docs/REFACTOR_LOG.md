...
- **Deferred (minor):** entry meta is saved even when a rename is refused; optimistic rename revert can clobber a concurrent edit to the same cell; `LATE_FILL_CLOSED` copy is unreachable from the UI; dashboard footer links inherit `ghostLink` 15px against a 13px row; dynamic-axis boards read as "not drawn" on the dashboard; a save conflict blocks `Review and publish` rather than the island's `Preview`; Playwright helpers are duplicated across three specs; `phase5-visual.spec.ts` names deleted selectors (skipped unless `PHASE5_CAPTURE`) — delete in stage 7.
