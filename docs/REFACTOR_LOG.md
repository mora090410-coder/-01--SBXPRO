...
- **Deferred (minor):** two article `h1`s lost an inline two-tone span (words intact); signup eyebrow repeats the h1; `Select squares` uses a changing name plus `aria-pressed`; touch `touch-none` in select mode blocks horizontal panning of the board on narrow screens (use `pan-x`); a rejected `onReload` after a successful published assign still reports "Nothing changed"; failed range apply moves focus to the toolbar instead of `Apply`; the organizer island in the homepage picture stays collapsed so `Draw numbers` is not visible; `phase5-accessibility.spec.ts` rewritten for the new login (stage 7 decides whether to keep it); Playwright helpers duplicated across specs.
