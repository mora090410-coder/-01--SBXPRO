...
- Payout descriptions: fields Q1, HALF (label `Halftime`), Q3, FINAL (label `Final`) with `maxLength={120}`, `notes` with `maxLength={280}`, placeholder `Winner gets bragging rights`, button `Save payout descriptions`, saved through the PATCH path (`onSavePayoutDescriptions`). No numeric inputs, no invented amounts.
...
// fields: Q1 `Q1`, HALF `Halftime`, Q3 `Q3`, FINAL `Final` (CapsuleInput, maxLength={120}, placeholder `Winner gets bragging rights`), notes `Board rules` (textarea, maxLength={280}); button `Save payout descriptions`; helper `GridOne never handles the money. Describe prizes in words.`
// ReconcileCard
