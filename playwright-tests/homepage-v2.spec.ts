...
test.describe('HomepageV2 Slice12 A1 browser surface', () => {
  ...
  test('flagged homepage satisfies phone first viewport and has no page overflow', async ({ page }) => {
    ...
    await expect(hero.getByText('Demonstration board — synthetic data')).toBeVisible();
    ...
    for (const locator of [
      ...
      hero.getByText('Demonstration board — synthetic data'),
    ]) {
    ...
    await hero.getByRole('button', { name: 'Viewer proof' }).click();
    await expect(hero.getByRole('heading', { name: 'Viewer proof' })).toBeVisible();
  ...
  test('desktop first viewport keeps required product truth and proof within 1280 by 720', async ({ page }) => {
    ...
    const required: Array<[string, Locator]> = [
      ...
      ['proof', hero.getByText('Demonstration board — synthetic data')],
    ];
  ...
  test('desktop can switch organizer proof to viewer proof in place', async ({ page }) => {
    ...
    await expect(proof.getByRole('heading', { name: /Organizer proof/i })).toBeVisible();
    await proof.getByRole('button', { name: 'Viewer proof' }).click();
    await expect(proof.getByRole('heading', { name: /Viewer proof/i })).toBeVisible();
  ...
  test('live-board CTA reaches the existing demo route', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Demo: Super Bowl LIX/i })).toBeVisible();
  });
