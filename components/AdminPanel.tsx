...
const AdminPanel: React.FC<AdminPanelProps> = ({ game, board, adminToken, activePoolId, onApply, onPublish }) => {
  ...
  const updateField = (field: keyof GameState, val: any) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setScanStatus(`Scan Failed: ${err.message || "Check API Key"}`);
    ...
    <div className="space-y-6">
      ...
      <div className="liquid-glass p-6 border-l-4 border-l-[#FFC72C] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-in slide-in-from-top-4 duration-500">
        ...
        <div className="flex gap-2 w-full md:w-auto items-center">
           {scanStatus && (
             <div className={`mr-4 text-[10px] font-black uppercase tracking-widest animate-pulse ${
               scanStatus.includes('SUCCESSFUL') ? 'text-green-400' : 
            ...
            <div className="min-w-[900px] space-y-4">
              ...
              <div className="flex items-end">
                ...
                <div className="flex-1">
                   ...
                   <div className="grid grid-cols-10 gap-2">
                     {parsedBoardData.oppAxis.map((val, idx) => (
                       <div key={idx} className="space-y-1.5">
                         ...
                         <div className="relative group">
                            <select 
                              ...
                              className="w-full bg-black/60 border border-white/10 rounded-lg p-2.5 text-center text-xs text-team-top font-black focus:border-team-top focus:bg-black/80 outline-none appearance-none transition-all shadow-inner cursor-pointer"
                            >
              ...
              <div className="flex">
                <div className="w-[120px] flex flex-col gap-2 pr-4 pt-1 border-r border-white/10">
                   {parsedBoardData.bearsAxis.map((val, idx) => (
                     <div key={idx} className="flex items-center justify-end gap-3 group h-12">
                       ...
                       <div className="relative w-12">
                          <select 
                            ...
                            className="w-full h-full bg-black/60 border border-white/10 rounded-lg p-2 text-center text-[11px] text-team-left font-black focus:border-team-left focus:bg-black/80 outline-none appearance-none transition-all cursor-pointer"
                          >
                            {axisDigits.map(d => <option key={d} value={d}>{d}</option>)}
                          </select>
                      ...
                      return (
                        <div key={cellIdx} className="relative group h-12">
                          <input 
                            ...
                            placeholder={`${parsedBoardData.oppAxis[c]}-${parsedBoardData.bearsAxis[r]}`}
