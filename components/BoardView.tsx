                ...
                <>
                    <div className="flex-1 flex flex-col relative z-50 w-full max-w-6xl mx-auto md:px-6 h-full">
                                ...
                                {(adminToken || isOwner) && (
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center bg-white/10 p-1 rounded-full border border-white/5 backdrop-blur-md">
                                            <button onClick={() => handleTogglePreview(false)} className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide transition-all ${!isPreviewMode ? 'bg-white text-black shadow-lg scale-105' : 'text-gray-400 hover:text-white'}`}>Edit</button>
                                            <button onClick={() => handleTogglePreview(true)} className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide transition-all ${isPreviewMode ? 'bg-white text-black shadow-lg scale-105' : 'text-gray-400 hover:text-white'}`}>Preview</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="flex-1 relative overflow-hidden flex flex-col pb-6">
                            <InfoCards.LiveStrip game={game} live={liveData} isSynced={isSynced} activeTab={activeTab} onTabChange={setActiveTab} />
            ...
            {isCommissionerMode && (
                <div className="fixed inset-0 z-[80] bg-[#050101] p-4 md:p-8 overflow-y-auto animate-in slide-in-from-bottom-10 duration-300">
                    <Suspense fallback={<div className="flex items-center justify-center h-full text-white/50">Loading Organizer...</div>}>
                        <AdminPanel game={game} board={board} adminToken={adminToken || ''} activePoolId={activePoolId || ''} liveData={liveData} onApply={(g, b) => { setGame(g); setBoard(b); }} onPublish={handlePublish} onClose={() => handleTogglePreview(true)} onLogout={handleLogout} onPreview={() => handleTogglePreview(true)} isActivated={isActivated} />
                    </Suspense>
                </div>
            )}
        </div>
